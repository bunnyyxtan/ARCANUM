...
function Shell({ children, onAction }: { children: ReactNode; onAction: () => void }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--wl-bg)] text-[var(--wl-ink)]">
      ...
      `}</style>
      <Header active="LEDGER" /><header className="hidden flex min-h-[68px] items-center justify-between border-b border-[var(--wl-line)] px-5 md:px-8">
        <div className="flex min-w-0 items-center gap-7">
          ...
          <nav className="hidden items-center gap-5 lg:flex">
            {links.map((link) => <a key={link} href={`#${link.toLowerCase()}`} className={`relative py-6 text-[12px] font-medium text-[var(--wl-body)] transition-colors hover:text-[var(--wl-ink)] ${link === "LEDGER" ? "text-[var(--wl-ink)] after:absolute after:bottom-[-1px] after:left-0 after:h-[2px] after:w-full after:bg-[var(--wl-signal)]" : ""}`}>{link}</a>)}
          </nav>
          <span className="font-mono text-[9px] uppercase tracking-[.15em] text-[var(--wl-mute)] lg:hidden">LEDGER</span>
        </div>
    ...
    <Shell onAction={() => action("Account controls are available to Helix-DAO operators.")}>
      <main className="mx-auto max-w-[1400px] px-5 py-8 md:px-8 md:py-10">
        <div className="flex flex-col justify-between gap-7 border-b border-[var(--wl-line)] pb-9 md:flex-row md:items-end">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[.18em] text-[var(--wl-signal)]">RECORD / LAST 24H</p>
            <h1 className="mt-4 text-[clamp(2.7rem,5vw,4.8rem)] font-semibold leading-[.9] tracking-[-.05em]">Governed ledger</h1>
            <p className="mt-4 max-w-[550px] text-[14px] leading-[1.45] text-[var(--wl-secondary2)]">A complete decision record for every governed movement across the Helix-DAO fleet.</p>
          </div>
          <button onClick={() => action("Ledger export queued: 24 hours of governed activity.")} className="arc-pill group w-fit rounded-full bg-[var(--wl-signal)] px-5 py-3 text-[11px] font-semibold text-[var(--wl-bg)]">Export report <span className="ml-2 transition-transform duration-[220ms] group-hover:translate-x-1">↗</span></button>
        </div>
        ...
        <section className="grid grid-cols-2 border-b border-[var(--wl-line)] md:grid-cols-4">
          {[["TOTAL VALUE", "$18,442.60", "money"], ["APPROVED", "126", "count"], ["REJECTED", "9", "count"], ["ESCALATED", "4", "count"]].map(([label, value, kind], index) => (
            <div key={label} className={`py-6 ${index > 0 ? "border-l border-[var(--wl-line)] pl-5 md:pl-7" : ""} ${index > 1 ? "border-t md:border-t-0" : ""}`}>
              <p className="font-mono text-[9px] tracking-[.15em] text-[var(--wl-mute)]">{label}</p>
              <p className={`mt-3 text-[clamp(1.5rem,3vw,2.15rem)] font-medium tracking-[-.05em] ${label === "ESCALATED" ? "text-[var(--wl-signal)]" : ""}`}>{kind === "money" ? <CountUp value={18442.6} prefix="$" decimals={2} /> : <CountUp value={Number(value)} />}</p>
            </div>
          ))}
        </section>
        ...
        <div className="flex flex-col gap-4 border-b border-[var(--wl-line)] py-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-wrap gap-2">
            {(["ALL", "APPROVED", "REJECTED", "ESCALATED", "FROZEN"] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-full border px-3.5 py-2 font-mono text-[9px] tracking-[.14em] transition-all duration-[220ms] ${filter === item ? "border-[var(--wl-ink)] bg-[var(--wl-ink)] text-[var(--wl-bg)]" : "border-[var(--wl-line)] text-[var(--wl-secondary2)] hover:border-[var(--wl-ink)] hover:text-[var(--wl-ink)]"}`}>{item}</button>)}
          </div>
          <label className="flex min-w-0 items-center gap-3 border-b border-[var(--wl-faint)] pb-2 text-[var(--wl-mute)] xl:w-[260px]">
            <span className="font-mono text-[10px]">⌕</span>
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="filter ledger rows..." className="min-w-0 flex-1 bg-transparent text-[12px] text-[var(--wl-ink)] outline-none placeholder:text-[var(--wl-mute)]" />
          </label>
        </div>
        ...
        <div className="mt-7 flex flex-col gap-7 xl:flex-row">
          <section className="min-w-0 flex-1 overflow-hidden border border-[var(--wl-line)] bg-[var(--wl-bg-raised)]">
            ...
            <div>
              {visibleRows.map((row, index) => <button key={`${row.time}-${row.agent}`} onClick={() => setSelected(row)} style={{ "--row-i": index } as CSSProperties} className={`arc-row grid w-full grid-cols-[1fr_auto] items-center gap-3 border-b border-[var(--wl-line-faint)] px-5 py-4 text-left last:border-b-0 md:grid-cols-[1.05fr_1.2fr_1fr_1fr_.9fr_auto] md:gap-4`}>
                <div><span className="font-mono text-[10px] tabular-nums text-[var(--wl-body)]">{row.time}</span><span className="ml-2 font-mono text-[9px] text-[var(--wl-mute)] md:hidden">UTC</span></div>
                <span className="text-[12px] font-medium">{row.agent}</span>
                <span className="hidden text-[12px] text-[var(--wl-body)] md:block">{row.counterparty}</span>
                <span className="hidden font-mono text-[10px] uppercase tracking-[.08em] text-[var(--wl-secondary2)] md:block">{row.category}</span>
                <span className="font-mono text-[12px] tabular-nums">{row.amount}</span>
                <StatusPill status={row.status} />
              </button>)}
              {visibleRows.length === 0 && <div className="px-6 py-16 text-center"><p className="font-mono text-[10px] uppercase tracking-[.14em] text-[var(--wl-signal)]">NO MATCHING RECORDS</p><p className="mt-3 text-[13px] text-[var(--wl-secondary2)]">Try another policy status or search term.</p></div>}
            </div>
            <div className="flex justify-between border-t border-[var(--wl-line)] px-5 py-3 font-mono text-[9px] text-[var(--wl-mute)]"><span>{visibleRows.length} visible records</span><span>policy/v4.18 · ARC / USDC</span></div>
          </section>
          ...
          <aside className="arc-drawer w-full shrink-0 border border-[var(--wl-line-bold)] bg-[var(--wl-bg-soft)] xl:w-[360px]">
            <div className="flex items-start justify-between border-b border-[var(--wl-line)] px-5 py-5">
              <div><p className="font-mono text-[9px] tracking-[.16em] text-[var(--wl-signal)]">DECISION RECORD</p><h2 className="mt-2 text-[18px] font-medium tracking-[-.04em]">{selected.agent}</h2></div>
              <StatusPill status={selected.status} />
            </div>
            <div className="px-5">
              <div className="border-b border-[var(--wl-line)] py-4"><p className="text-[13px] leading-[1.45] text-[var(--wl-body)]">{selected.decision}</p></div>
              <dl className="divide-y divide-[var(--wl-line)] font-mono text-[10px]">
                {[
                  ...
                  ["TX HASH", selected.tx],
                ].map(([label, value]) => <div key={label} className="grid grid-cols-[1fr_1.2fr] gap-3 py-3"><dt className="text-[var(--wl-mute)]">{label}</dt><dd className="break-words text-right text-[var(--wl-body)]">{value}</dd></div>)}
              </dl>
              <div className="border-t border-[var(--wl-line)] py-4"><p className="font-mono text-[9px] tracking-[.12em] text-[var(--wl-mute)]">RAW CALLDATA NOTE</p><p className="mt-2 text-[12px] text-[var(--wl-body)]">312 bytes · policy/v4.18 evaluation payload</p></div>
              <div className="flex flex-wrap gap-2 pb-5">
                <button onClick={() => action(`Arcscan opened for ${selected.tx}.`)} className="arc-pill arc-ghost rounded-full border border-[var(--wl-line)] px-3.5 py-2.5 text-[10px] font-semibold">View on Arcscan ↗</button>
                <button onClick={() => action(`Vendor flag created for ${selected.counterparty}.`)} className="rounded-full border border-[var(--wl-signal)] px-3.5 py-2.5 text-[10px] font-semibold text-[var(--wl-signal)] transition-colors duration-[220ms] hover:bg-[var(--wl-signal)] hover:text-[var(--wl-bg)]">Flag vendor</button>
              </div>
            </div>
          </aside>
        </div>

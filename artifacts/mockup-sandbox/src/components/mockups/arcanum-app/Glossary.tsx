...
function StatusMark({ children }: { children: ReactNode }) {
  return <span className="rounded-full bg-[var(--wl-green-tint)] px-2.5 py-1 font-mono text-[9px] tracking-[.12em] text-[var(--wl-green)]">{children}</span>;
}
      ...
      <div className="mx-auto max-w-[1400px] px-5 py-9 md:px-8 md:py-12">
        <div className="flex flex-col justify-between gap-8 border-b border-[var(--wl-line)] pb-10 lg:flex-row lg:items-end">
          <div>
            ...
            <h1 className="mt-5 text-[clamp(3.2rem,7vw,6.6rem)] font-semibold leading-[.84] tracking-[-.045em]">The vocabulary<br /><span className="text-[var(--wl-dim)]">of accountable spend.</span></h1>
            <p className="mt-7 max-w-[540px] text-[15px] leading-[1.5] text-[var(--wl-body)]">Terms for operators who need the same word to mean the same control. Browse the language behind HELIX-DAO's governed wallets.</p>
          </div>
          <div className="max-w-[250px] border-l border-[var(--wl-signal)] pl-4">
            <p className="font-mono text-[9px] uppercase tracking-[.16em] text-[var(--wl-signal)]">INDEX / 10 TERMS</p>
            <p className="mt-3 text-[12px] leading-[1.5] text-[var(--wl-secondary2)]">A living reference. Doctrines change; the meaning of a recorded decision should not.</p>
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_390px]">
          <section>
            <div className="flex flex-col gap-5 border-b border-[var(--wl-line)] pb-5 md:flex-row md:items-center md:justify-between">
              <label className="flex items-center gap-3 border-b border-[var(--wl-faint)] pb-2 md:w-[300px]">
                ...
                <input aria-label="Search glossary" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search terms" className="w-full bg-transparent text-[13px] outline-none placeholder:text-[var(--wl-mute)]" />
              </label>
              <div className="flex flex-wrap gap-1.5">
                {letters.map((item) => <button type="button" key={item} onClick={() => setLetter(item)} className={`glossary-letter rounded-full px-2.5 py-1.5 font-mono text-[9px] tracking-[.12em] ${letter === item ? "bg-[var(--wl-ink)] text-[var(--wl-bg)]" : "text-[var(--wl-secondary)]"}`}>{item}</button>)}
              </div>
            </div>
            <div className="mt-5 flex items-center justify-between">
              <p className="font-mono text-[10px] uppercase tracking-[.15em] text-[var(--wl-mute)]">{visibleTerms.length} MATCHES / ALPHABETICAL</p>
              <a data-nav="DOCS" href="#docs" className="font-mono text-[10px] uppercase tracking-[.14em] text-[var(--wl-body)] underline decoration-[var(--wl-signal)] underline-offset-4 hover:text-[var(--wl-signal)]">Read the field manual</a>
            </div>
            <div className="mt-3 divide-y divide-[var(--wl-line-soft)] border-y border-[var(--wl-line)]">
              {visibleTerms.length ? visibleTerms.map((item) => <button type="button" key={item.term} data-active={active.term === item.term} onClick={() => setSelected(item.term)} className="glossary-row grid w-full grid-cols-[40px_1fr] gap-3 px-3 py-5 text-left md:grid-cols-[46px_170px_1fr] md:items-center"><span className="font-mono text-[10px] text-[var(--wl-signal)]">{item.letter}</span><span className="text-[16px] font-medium tracking-[-.02em]">{item.term}</span><span className="text-[12px] leading-[1.45] text-[var(--wl-secondary2)]">{item.definition}</span></button>) : <div className="border border-dashed border-[var(--wl-faint)] px-5 py-12 text-center"><p className="font-mono text-[10px] uppercase tracking-[.15em] text-[var(--wl-secondary)]">NO TERMS FOUND</p><p className="mt-3 text-[13px] text-[var(--wl-secondary2)]">Clear the search or return to the full index.</p></div>}
            </div>
            <div className="flex justify-between pt-4 font-mono text-[9px] uppercase tracking-[.12em] text-[var(--wl-mute)]"><span>HELIX-DAO · ARC TESTNET</span><span>INDEXED / V4.18</span></div>
          </section>
          ...
          <aside className="h-fit border border-[var(--wl-line-bold)] bg-[var(--wl-bg-raised)] shadow-[12px_14px_0_var(--wl-line-faint)] lg:sticky lg:top-5">
            <div className="border-b border-[var(--wl-line)] px-6 py-5">
              <div className="flex items-center justify-between"><p className="font-mono text-[9px] uppercase tracking-[.17em] text-[var(--wl-signal)]">SELECTED ENTRY</p><StatusMark>DEFINED</StatusMark></div>
            </div>
            <div key={active.term} className="glossary-detail px-6 py-7">
              <p className="font-mono text-[11px] text-[var(--wl-signal)]">{active.letter} / 0{terms.indexOf(active) + 1}</p>
              ...
              <div className="mt-7 border-t border-[var(--wl-line)] pt-5">
                ...
                <p className="mt-3 text-[13px] leading-[1.55] text-[var(--wl-body)]">{active.detail}</p>
              </div>
              <div className="mt-7 border-t border-[var(--wl-line)] pt-5">
                <p className="font-mono text-[9px] uppercase tracking-[.16em] text-[var(--wl-mute)]">SEE ALSO</p>
                <div className="mt-3 flex flex-wrap gap-2">{active.related.map((item) => <button type="button" key={item} onClick={() => setSelected(item)} className="rounded-full border border-[var(--wl-line)] px-3 py-1.5 text-[11px] text-[var(--wl-body)] transition-colors hover:border-[var(--wl-signal)] hover:text-[var(--wl-signal)]">{item}</button>)}</div>
              </div>
            </div>
            <div className="border-t border-[var(--wl-line)] bg-[var(--wl-bg-soft)] px-6 py-4 font-mono text-[9px] uppercase leading-[1.7] tracking-[.1em] text-[var(--wl-secondary)]">TERM OWNER / ARCANUM<br />LAST REVIEW / 2025-08-14</div>
          </aside>
        </div>

        <div className="mt-20 flex flex-col justify-between gap-6 border-t border-[var(--wl-line)] pt-8 md:flex-row md:items-end">
          <div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[var(--wl-signal)]">KEEP READING</p><h2 className="mt-3 text-[26px] font-medium tracking-[-.05em]">The words become useful in the field.</h2></div>
          <a data-nav="DOCS" href="#docs" className="inline-flex w-fit rounded-full bg-[var(--wl-signal)] px-5 py-3 text-[11px] font-semibold text-white transition-transform duration-[220ms] hover:-translate-y-0.5 hover:bg-[var(--wl-signal-deep)]">Open the field manual <span className="ml-2">↗</span></a>
        </div>
      </div>
    </main>
  );

        ...
        <div className="divide-y divide-[#e3dcd5] border-b border-[#ded7d0]">
          ...
          {anomalies.map((item, index) => (
            <div key={item.agent} style={{ "--row": index } as CSSProperties} className="anomaly-row grid gap-3 px-4 py-5 md:grid-cols-[1.08fr_1fr_.72fr_.55fr_1.4fr_.75fr_1.55fr] md:items-center">
              ...
              <div className="flex items-center justify-start gap-3 md:justify-end">{item.status === "FROZEN" ? <span className="inline-flex items-center gap-1.5 rounded-full bg-[#292522] px-3 py-1.5 font-mono text-[9px] tracking-[.1em] text-[#faf6f1]"><span className="h-1.5 w-1.5 rounded-full bg-[#ff3c00]" />RESTRAINED</span> : <button type="button" onClick={() => restrain(item.agent)} className="rounded-full border border-[#ff3c00] px-3 py-1.5 font-mono text-[9px] tracking-[.1em] text-[#ff3c00] transition-all duration-[220ms] hover:-translate-y-0.5 hover:bg-[#ff3c00] hover:text-white">Restrain</button>}<button type="button" onClick={() => { const opening = investigated !== item.agent; setInvestigated(opening ? item.agent : null); if (opening) setNotice(`${item.agent.toUpperCase()} INVESTIGATION OPEN · 4 TRANSACTIONS FLAGGED`); }} className="warm-pill warm-pill-ghost rounded-full border border-[#ded7d0] px-3 py-1.5 font-mono text-[9px] tracking-[.1em]">{investigated === item.agent ? "Close trace" : "Investigate"}</button><button type="button" onClick={() => dismiss(item.agent)} className="font-mono text-[9px] tracking-[.1em] text-[#837a72] transition-colors hover:text-[#292522]">Dismiss</button></div>
              {investigated === item.agent && <div className="md:col-span-7 border-l-2 border-[#ff3c00] bg-[#f5f0ea] px-5 py-4">
                <div className="flex items-center justify-between"><p className="font-mono text-[9px] uppercase tracking-[.16em] text-[#ff3c00]">INVESTIGATION TRACE / {item.agent.toUpperCase()}</p><span className="font-mono text-[8.5px] tracking-[.1em] text-[#9b9289]">OPENED {item.time}</span></div>
                <div className="mt-3 grid gap-x-8 gap-y-2 text-[11px] text-[#655d56] sm:grid-cols-2">
                  <div className="flex justify-between border-b border-[#e3dcd5] pb-1.5"><span>Baseline comparison</span><span className="font-mono text-[9px] tracking-[.08em] text-[#ff3c00]">DEVIATION {item.score}σ</span></div>
                  <div className="flex justify-between border-b border-[#e3dcd5] pb-1.5"><span>Vendor path</span><span className="font-mono text-[9px] tracking-[.08em] text-[#3f653e]">3 HOPS VERIFIED</span></div>
                  <div className="flex justify-between border-b border-[#e3dcd5] pb-1.5"><span>Wallet history · 30d</span><span className="font-mono text-[9px] tracking-[.08em] text-[#655d56]">4 TX FLAGGED</span></div>
                  <div className="flex justify-between border-b border-[#e3dcd5] pb-1.5"><span>Policy doctrine check</span><span className="font-mono text-[9px] tracking-[.08em] text-[#ff3c00]">§3.2 CAP BURST</span></div>
                </div>
                <div className="mt-3 flex gap-4"><button type="button" data-nav="LEDGER" className="font-mono text-[9px] tracking-[.12em] text-[#ff3c00] hover:underline">OPEN LEDGER ROWS →</button><button type="button" data-nav="AGENT_DETAIL" className="font-mono text-[9px] tracking-[.12em] text-[#837a72] hover:text-[#292522]">VIEW DOSSIER</button></div>
              </div>}
            </div>
          ))}
        </div>

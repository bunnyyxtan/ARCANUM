...
function ShellLink({ active, children }: { active?: boolean; children: ReactNode }) {
  return (
    <button
      type="button"
      onClick={() => undefined}
      className={`relative h-full whitespace-nowrap px-3 text-[12px] font-medium tracking-[-.01em] transition-colors duration-[220ms] ${active ? "text-[var(--wl-ink)]" : "text-[var(--wl-body)] hover:text-[var(--wl-ink)]"}`}
    >
      {children}
      {active && <span className="absolute bottom-[-1px] left-3 right-3 h-[2px] bg-[var(--wl-signal)]" />}
    </button>
  );
}
    ...
    <main className="min-h-[100dvh] bg-[var(--wl-bg)] text-[var(--wl-ink)]" style={{ fontFamily: "'Inter Tight', sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box}.font-mono{font-family:'DM Mono',monospace}
        ...
        .health-card{transition:transform 220ms cubic-bezier(.16,1,.3,1),box-shadow 220ms ease}.health-card:hover{transform:translateY(-2px);box-shadow:0 12px 24px -20px rgba(var(--wl-ink-rgb),.5)}
        ...
        .data-row{transition:transform 220ms cubic-bezier(.16,1,.3,1),background-color 220ms ease}.data-row:hover{transform:translateX(3px);background:var(--wl-bg-soft)}
      ...
      `}</style>
      <Header active="STATUS" /><header className="hidden flex h-[68px] items-center justify-between border-b border-[var(--wl-line)] px-8">
        ...
        <div className="hidden shrink-0 items-center gap-5 md:flex">
          ...
          <button type="button" onClick={() => undefined} className="flex items-center gap-2 rounded-full border border-[var(--wl-line)] px-3 py-1.5 transition-colors duration-[220ms] hover:border-[var(--wl-ink)]">
            <span className="flex h-[22px] w-[22px] items-center justify-center rounded-full bg-[var(--wl-ink)] font-mono text-[9px] text-[var(--wl-bg)]">HD</span><span className="text-[12px] font-medium">HELIX-DAO</span>
          </button>
        </div>
      </header>
      {children}
...
function HealthCard({ index, label, detail, metric, metricLabel, status = "OPERATIONAL" }: { index: number; label: string; detail: string; metric: string; metricLabel: string; status?: string }) {
  return (
    <article className="health-card warm-reveal relative border-r border-[var(--wl-line)] px-7 py-7 first:pl-0 last:border-r-0 last:pr-0 max-lg:border-b max-lg:border-r-0 max-lg:px-0 max-lg:py-6" style={{ "--i": index } as CSSProperties}>
      <span className="card-rule absolute left-0 right-7 top-0 h-[2px] bg-[var(--wl-signal)]" />
      <div className="flex items-start justify-between gap-5">
        <span className="font-mono text-[10px] uppercase tracking-[.16em] text-[var(--wl-body)]">{label}</span>
        <span className={`font-mono text-[10px] tracking-[.12em] ${status === "OPERATIONAL" ? "text-[var(--wl-ink)]" : "text-[var(--wl-signal)]"}`}>{status}</span>
      </div>
      <p className="mt-12 max-w-[290px] text-[13px] leading-[1.45] text-[var(--wl-secondary2)]">{detail}</p>
      <div className="mt-8 border-t border-[var(--wl-line-soft)] pt-4">
        ...
        <span className="mt-1 block font-mono text-[9px] uppercase tracking-[.14em] text-[var(--wl-mute)]">{metricLabel}</span>
      </div>
    </article>
  );
}
      ...
      <div className="app-content mx-auto max-w-[1400px] px-8 py-10">
        <div className="flex items-end justify-between gap-8 max-md:flex-col max-md:items-start">
          <div className="warm-reveal">
            <p className="font-mono text-[10px] uppercase tracking-[.2em] text-[var(--wl-signal)]">SYSTEMS / READ MODEL</p>
            <h1 className="mt-5 text-[clamp(3rem,6vw,5.8rem)] font-semibold leading-[.85] tracking-[-.045em]">Status</h1>
            <p className="mt-6 max-w-[500px] text-[14px] leading-[1.5] text-[var(--wl-secondary2)]">A direct read on the services that keep governed wallets accountable.</p>
          </div>
          <button type="button" onClick={runCheck} className="warm-pill group rounded-full bg-[var(--wl-signal)] px-5 py-3 text-[12px] font-semibold text-[#fff]">
        ...
        <section className="warm-reveal mt-16 grid gap-12 border border-[var(--wl-line)] bg-[var(--wl-bg-soft)] p-8 md:grid-cols-[1fr_280px] md:p-10" style={{ "--i": 4 } as CSSProperties}>
          <div className="max-w-[700px]">
            <p className="font-mono text-[10px] uppercase tracking-[.18em] text-[var(--wl-signal)]">READ MODEL / HOW TO READ THIS</p>
            <p className="mt-8 text-[16px] leading-[1.5] text-[var(--wl-body)]">Supabase stores wallet-creation writes so the dashboard can answer quickly. The indexer tracks on-chain history and may lag behind the latest Arc block.</p>
            <p className="mt-5 text-[16px] leading-[1.5] text-[var(--wl-body)]">Fresh wallets may show no indexed activity until their first transactions are picked up. That is expected—not a missing policy decision.</p>
          </div>
          <div className="flex flex-col justify-between border-l border-[var(--wl-line)] pl-6 max-md:border-l-0 max-md:border-t max-md:pl-0 max-md:pt-6">
            <span className="font-mono text-[9px] uppercase tracking-[.16em] text-[var(--wl-mute)]">LAST CHECKED</span>
            <span className="mt-4 font-mono text-[12px] tabular-nums text-[var(--wl-ink)]">{checkedAt}</span>
            <span className="mt-8 text-[11px] leading-[1.45] text-[var(--wl-secondary)]">All read-only checks complete without writing to the ledger.</span>
          </div>
        </section>
      </div>
    </AppShell>

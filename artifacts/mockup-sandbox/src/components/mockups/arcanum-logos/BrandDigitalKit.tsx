...
function IconProof({ size, darkGround = false }: { size: number; darkGround?: boolean }) {
  return <div className={`icon-ground ${darkGround ? "dark" : ""}`}><ArchwayCoin size={size} coin={darkGround ? paper : ink} paper={darkGround ? dark : paper} signal={darkGround ? "#ff5a1f" : signal} /></div>;
}
...
export default function BrandDigitalKit() {
  return <main className="brand-plate"><style>{style}</style><header className="brand-head"><span>ARCANUM BRAND — DIGITAL KIT</span><span>APPLICATIONS · 02</span></header>
    ...
    <section className="digital-section"><div className="digital-label"><span>FAVICON / APP ICON PROOF</span><span>LIGHT + DARK GROUND</span></div><div className="icon-row">{[16,32,64,180].map(size=><div className="icon-item" key={`light-${size}`}><IconProof size={size}/><span>{size} / paper</span></div>)}{[16,32,64,180].map(size=><div className="icon-item" key={`dark-${size}`}><IconProof size={size} darkGround/><span>{size} / foundry</span></div>)}</div></section>
    ...
    <section className="digital-section"><div className="digital-label"><span>OPEN GRAPH / 1200 × 630</span><span>DARK FOUNDRY</span></div><div className="og-card"><div className="og-copy"><div className="og-title">Money with<br/>a policy layer.</div><div className="og-label">ARCANUM · GOVERNED TREASURY</div></div><div className="og-mark"><ArchwayCoinReversed size={122}/></div></div></section>

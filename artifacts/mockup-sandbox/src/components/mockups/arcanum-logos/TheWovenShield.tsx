...
function WovenArt({ detail = false }: { detail?: boolean }) {
  ...
  const left = Array.from({ length: 13 }, (_, i) => {
    const x = 48 + i * 25;
    return <path key={`left-${i}`} d={`M${x} 42C${x + 38} 126 ${x + 58} 250 ${x + 82} 364`} fill="none" stroke={i % 3 === 0 ? GOLD : SHADOW} strokeWidth={i % 3 === 0 ? 1.15 : 0.7} />;
  });
  const right = Array.from({ length: 13 }, (_, i) => {
    const x = 48 + i * 25;
    return <path key={`right-${i}`} d={`M${x + 82} 42C${x + 44} 126 ${x + 24} 250 ${x} 364`} fill="none" stroke={i % 3 === 0 ? GOLD : SHADOW} strokeWidth={i % 3 === 0 ? 1.15 : 0.7} />;
  });
  const glory = Array.from({ length: 18 }, (_, i) => <path key={`glory-${i}`} d="M200 22V6" transform={`rotate(${(i - 9) * 4} 200 200)`} stroke={SHADOW} strokeWidth="0.8" />);
  return (
    <g>
      <defs><clipPath id={detail ? "woven-detail-clip" : "woven-clip"}><path d={innerShield} /></clipPath></defs>
      <g>{glory}</g>
      <path d={outerShield} fill="none" stroke={SHADOW} strokeWidth="2" />
      <path d={outerShield} fill="none" stroke={GOLD} strokeWidth="0.9" />
      <path d={innerShield} fill="none" stroke={GOLD} strokeWidth="1.5" />
      <g clipPath={`url(#${detail ? "woven-detail-clip" : "woven-clip"})`} opacity="0.92">{left}{right}</g>
      <circle cx="200" cy="92" r="9.5" fill="none" stroke={PALE} strokeWidth="2.2" />
      <path d="M194 99H206V130H194Z" fill={PALE} />
      <path d="M200 58V344" stroke={SHADOW} strokeWidth="0.65" opacity="0.7" />
    </g>
  );
}
...
export default function TheWovenShield() {
  return (
    <main style={{ minHeight: "100dvh", width: "100%", boxSizing: "border-box", background: BG, color: GOLD, padding: "27px 30px 28px", display: "flex", flexDirection: "column", fontFamily: MONO }}>
      ...
      <p style={{ width: "100%", maxWidth: 640, margin: "10px auto 0", color: SHADOW, fontSize: 9, lineHeight: 1.35, letterSpacing: "0.12em", textTransform: "uppercase" }}>A POLICY MESH MONEY CANNOT SLIP THROUGH. VIGILANCE LIVES IN THE CHIEF.</p>
    </main>
  );
}
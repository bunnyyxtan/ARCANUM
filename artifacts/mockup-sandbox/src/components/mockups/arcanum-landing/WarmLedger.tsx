...
function LedgerRows({ dark = false }: { dark?: boolean }) {
  ...
  const line = dark ? "border-[#554d47]" : "border-[#ded7d0]";
  const quiet = dark ? "text-[#8f857c]" : "text-[#9b9289]";
  return (
    <div className={`overflow-hidden border ${line} ${dark ? "bg-[#322e2b]" : "bg-[#fbf8f4]"}`}>
      ...
      <div className={`divide-y ${dark ? "divide-[#4b4540]" : "divide-[#e7e0d9]"}`}>
        {rows.map((r, i) => <div key={r.time} style={{ "--row-i": i } as CSSProperties} className={`warm-ledger-row grid grid-cols-[1.3fr_1fr_.9fr_1fr_auto] items-center gap-3 px-5 py-4 ${i < visible ? "is-live" : "is-quiet"}`}>
          <div><div className={`text-[12px] font-medium ${dark ? "text-[#f3ede7]" : "text-[#292522]"}`}>{r.agent}</div><div className={`mt-1 font-mono text-[9px] ${quiet}`}>{r.time} UTC</div></div>
          <span className={`text-[12px] ${dark ? "text-[#c5bbb2]" : "text-[#5b544d]"}`}>{r.vendor}</span><span className={`font-mono text-[12px] tabular-nums ${dark ? "text-[#f3ede7]" : "text-[#292522]"}`}>{r.amount}</span><span className={`font-mono text-[10px] ${quiet}`}>{r.detail}</span><StatusPill status={r.status} />
        </div>)}
      </div>
      <div className={`flex items-center justify-between border-t px-5 py-3 ${line}`}><span className={`font-mono text-[9px] ${quiet}`}>policy/v4.18 · 42ms median</span><span className="font-mono text-[9px] text-[#72946f]">● streaming</span></div>
    </div>
  );
}
...
export function WarmLedger() {
  ...
  return (
    <main className="min-h-[100dvh] overflow-hidden bg-[#faf6f1] text-[#292522]" style={{ fontFamily: "'Inter Tight', sans-serif" }}>
      ...
      <aside className="fixed bottom-0 left-0 top-0 z-20 hidden w-[86px] flex-col items-center justify-between border-r border-[#ded7d0] bg-[#faf6f1] py-7 lg:flex">
        <MagneticAnchor href="#" className="text-[17px] font-bold tracking-[-.045em]" >ARCANUM<span className="text-[#ff3c00]">.</span></MagneticAnchor>
        <div className="font-mono text-[9px] tracking-[.18em] text-[#9b9289]" style={{ writingMode: "vertical-rl" }}>ARC · GOVERNED</div>
        <div ref={railRef} className="relative flex flex-col items-center gap-6 font-mono text-[9px] text-[#837a72]"><span className="warm-active-dot" aria-hidden="true" /><MagneticAnchor href="#governed" className="warm-nav-dot" >01</MagneticAnchor><MagneticAnchor href="#policies" className="warm-nav-dot" >02</MagneticAnchor><MagneticAnchor href="#record" className="warm-nav-dot" >03</MagneticAnchor><MagneticAnchor href="#contact" className="warm-nav-dot" >04</MagneticAnchor></div>
      </aside>

      <div className="lg:pl-[86px]">
        <nav className="flex items-center justify-between border-b border-[#ded7d0] px-6 py-5 lg:hidden"><MagneticAnchor href="#" className="text-[18px] font-bold tracking-[-.05em]">ARCANUM<span className="text-[#ff3c00]">.</span></MagneticAnchor><MagneticAnchor href="/dashboard" className="warm-pill group rounded-full bg-[#ff3c00] px-5 py-2.5 text-[11px] font-semibold text-white">Launch Dashboard<Arrow /></MagneticAnchor></nav>
        <div className="hidden h-[68px] items-center justify-between border-b border-[#ded7d0] pl-10 pr-6 lg:flex"><span className="text-[13px] font-medium tracking-[-.01em] text-[#655d56]">Governed wallets for AI agents<span className="text-[#9b9289]"> · USDC on Arc</span></span><div className="flex items-center"><MagneticAnchor href="/docs" className="warm-link mx-4 inline-flex items-center gap-2 text-[13px] font-medium tracking-[-.01em] text-[#655d56] transition-colors hover:text-[#292522]"><BookIcon className="h-[15px] w-[15px] text-[#9b9289]" />Read Docs</MagneticAnchor><MagneticAnchor href="https://github.com/bunnyyxtan/ARCANUM" target="_blank" rel="noreferrer" className="warm-link mx-4 inline-flex items-center gap-2 text-[13px] font-medium tracking-[-.01em] text-[#655d56] transition-colors hover:text-[#292522]"><GitHubMark className="h-[15px] w-[15px] text-[#9b9289]" />GitHub</MagneticAnchor><span className="mx-4 h-5 w-px bg-[#ded7d0]" aria-hidden="true" /><MagneticAnchor href="/dashboard" className="warm-pill group ml-2 rounded-full bg-[#ff3c00] px-6 py-2.5 text-[13px] font-semibold tracking-[-.01em] text-white">Launch Dashboard<Arrow /></MagneticAnchor></div></div>

        <section id="ledger" className="relative min-h-[780px] border-b border-[#ded7d0] px-6 py-16 lg:px-10 lg:py-20">
          <div ref={gridRef} className="pointer-events-none absolute inset-0 opacity-60 will-change-transform" style={{ backgroundImage: "linear-gradient(#e9e2db 1px, transparent 1px), linear-gradient(90deg, #e9e2db 1px, transparent 1px)", backgroundSize: "72px 72px", maskImage: "linear-gradient(to bottom, black, transparent 80%)" }} />
          <div className="relative mx-auto max-w-[1400px]">
            <Reveal><p className="font-mono text-[10px] uppercase tracking-[.22em] text-[#ff3c00]">Governed autonomy / Arc blockchain</p></Reveal>
            <Reveal kind="headline" className="delay-1"><h1 className="mt-8 max-w-[880px] text-[clamp(4.4rem,10.5vw,10.5rem)] font-semibold leading-[.8] tracking-[-.045em]">Autonomous<br /><span className="text-[#8d837b]">spend,</span> <em className="not-italic text-[#292522]">accounted.</em></h1></Reveal>
            <Reveal className="delay-2"><div className="mt-12"><p className="max-w-[420px] text-[16px] leading-[1.45] text-[#655d56]">Every dollar is checked, recorded, and visibly governed before it moves.</p><div className="mt-7 flex flex-wrap items-center gap-4"><MagneticAnchor href="/dashboard" className="warm-pill group rounded-full bg-[#ff3c00] px-6 py-3.5 text-[12px] font-semibold text-white">Launch Dashboard<Arrow /></MagneticAnchor><MagneticAnchor href="/docs" className="warm-pill warm-pill-ghost inline-flex items-center gap-2 rounded-full border border-[#ded7d0] px-6 py-3.5 text-[12px] font-semibold text-[#292522]"><BookIcon />Read Docs</MagneticAnchor><MagneticAnchor href="https://github.com/bunnyyxtan/ARCANUM" target="_blank" rel="noreferrer" className="warm-link inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[.12em] text-[#837a72] transition-colors hover:text-[#292522]"><GitHubMark />GitHub ↗</MagneticAnchor></div></div></Reveal>
            <div className="relative mt-20 lg:mt-24 lg:ml-[15%]"><Reveal className="delay-3"><LedgerRows /></Reveal><div className="absolute bottom-full right-0 mb-5 hidden w-[200px] border-l border-[#ff3c00] pl-4 text-[10px] leading-[1.4] text-[#ff3c00] lg:block">THE LIVE RECORD<br /><span className="text-[#837a72]">Not a demo. A transaction deciding itself in public.</span><strong className="mt-4 block font-mono text-[20px] font-medium tabular-nums text-[#292522]">${capitalGoverned.toFixed(1)}k</strong><span className="block font-mono text-[8px] uppercase tracking-[.12em] text-[#837a72]">capital governed</span></div></div>
          </div>
        </section>

        <section id="governed" className="border-b border-[#ded7d0] bg-[#f5f0ea] px-6 py-24 lg:px-10 lg:py-32"><div className="mx-auto grid max-w-[1400px] gap-16 lg:grid-cols-[240px_1fr]"><Reveal><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#ff3c00]">01 / The control loop</p><SectionNumber value="1" className="mt-28 hidden font-mono text-[80px] leading-none tracking-[-.045em] text-[#d8d0c8] lg:block" /></Reveal><Reveal className="delay-1"><h2 className="max-w-[700px] text-[clamp(3rem,6vw,6.2rem)] font-semibold leading-[.84] tracking-[-.045em]">A dollar earns<br />its way through.</h2><div className="mt-20 grid border-t border-[#bdb4aa] md:grid-cols-3">{[["01","Policy check","Caps, vendors, destinations. Evaluated in 42ms."],["02","Allow or block","The wallet moves only when the policy says so."],["03","Escalate to human","Unusual spend pauses. You decide, not the model."]].map(([n,t,d]) => <div key={n} className="border-b border-[#bdb4aa] py-7 md:border-b-0 md:border-r md:px-8 md:pt-8 md:first:pl-0 md:last:border-r-0 md:last:pr-0"><span className="font-mono text-[10px] text-[#ff3c00]">{n}</span><h3 className="mt-14 text-[21px] font-medium tracking-[-.04em]">{t}</h3><p className="mt-3 max-w-[190px] text-[12px] leading-[1.45] text-[#776f68]">{d}</p></div>)}</div></Reveal></div></section>

        <section id="policies" className="border-b border-[#ded7d0] px-6 py-28 lg:px-10 lg:py-36"><div className="mx-auto max-w-[1400px]"><Reveal><div className="flex items-start justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#ff3c00]">02 / Policy surface</p><h2 className="mt-7 text-[clamp(3rem,6vw,6rem)] font-semibold leading-[.84] tracking-[-.045em]">The rulebook,<br /><span className="text-[#8d837b]">made executable.</span></h2></div><SectionNumber value="2" className="hidden font-mono text-[80px] leading-none tracking-[-.045em] text-[#e3dcd5] lg:block" /></div></Reveal>
           <Reveal className="delay-1"><div className="warm-policy-doc relative mt-24 max-w-[930px] border border-[#cfc5bc] bg-[#fbf8f4] p-7 shadow-[14px_18px_0_#eee7df] lg:ml-[12%] lg:p-12"><div className="flex justify-between border-b border-[#ded7d0] pb-5 font-mono text-[9px] uppercase tracking-[.16em] text-[#9b9289]"><span>POLICY / PROCUREMENT-BOT</span><span>v4.18 · ACTIVE</span></div><div className="mt-10 grid gap-9 font-mono text-[12px] leading-[1.8] md:grid-cols-[1fr_1fr]"><div><span className="text-[#9b9289]">when</span><br /><span className="text-[#ff3c00]">transaction.vendor</span> in<br /><span className="ml-5">[ AWS, OpenAI ]</span><br /><span className="text-[#ff3c00]">and amount</span> ≤ <span className="text-[#ff3c00]">$500</span></div><div><span className="text-[#9b9289]">then</span><br /><span className="text-[#4d7b49]">ALLOW</span> · record to ledger<br /><span className="text-[#9b9289]">otherwise</span><br /><span className="text-[#ff3c00]">ESCALATE</span> · ask human operator</div></div><div className="mt-10 flex justify-between border-t border-[#ded7d0] pt-5 text-[10px] text-[#837a72]"><span>daily cap $5,000 · USDC only</span><span className="text-[#4d7b49]">✓ signed by operator</span></div></div></Reveal>
        </div></section>

        <section id="record" className="bg-[#292522] px-6 py-28 text-[#faf6f1] lg:px-10 lg:py-32"><div className="mx-auto grid max-w-[1400px] gap-16 lg:grid-cols-[240px_1fr]"><Reveal><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#ff3c00]">03 / Trust surface</p><SectionNumber value="3" className="mt-28 hidden font-mono text-[80px] leading-none tracking-[-.045em] text-[#49423d] lg:block" /></Reveal><Reveal className="delay-1"><div className="flex flex-col justify-between gap-10 md:flex-row md:items-end"><h2 className="max-w-[680px] text-[clamp(3rem,6vw,6rem)] font-semibold leading-[.84] tracking-[-.045em]">Nothing moves<br /><span className="text-[#8f857c]">in the dark.</span></h2><p className="max-w-[240px] text-[12px] leading-[1.5] text-[#a69d94]">A quiet, immutable record of what your agents tried, what policy decided, and who stepped in.</p></div><div className="mt-16"><LedgerRows dark /></div></Reveal></div></section>

        <section id="contact" className="px-6 py-32 lg:px-10 lg:py-44"><Reveal><div className="mx-auto max-w-[1400px] border-b border-[#ded7d0] pb-24"><div className="flex items-start justify-between"><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#ff3c00]">04 / Trust is the product</p><SectionNumber value="4" className="hidden font-mono text-[80px] leading-none tracking-[-.045em] text-[#e3dcd5] lg:block" /></div><h2 className="mt-8 max-w-[950px] text-[clamp(4rem,9vw,9rem)] font-semibold leading-[.78] tracking-[-.045em]">Let agents move.<br /><span className="text-[#8d837b]">Keep the final word.</span></h2><div className="mt-14 flex flex-col gap-8 lg:ml-[42%] lg:flex-row lg:items-center"><p className="max-w-[290px] text-[15px] leading-[1.5] text-[#655d56]">Built for finance and engineering teams who need autonomy without giving up the ledger.</p><div className="flex flex-wrap items-center gap-4"><MagneticAnchor href="/dashboard" className="warm-pill group w-fit rounded-full bg-[#ff3c00] px-6 py-3.5 text-[12px] font-semibold text-white">Launch Dashboard<Arrow /></MagneticAnchor><MagneticAnchor href="/docs" className="warm-pill warm-pill-ghost inline-flex w-fit items-center gap-2 rounded-full border border-[#ded7d0] px-6 py-3.5 text-[12px] font-semibold text-[#292522]"><BookIcon />Read Docs</MagneticAnchor></div></div></div></Reveal></section>

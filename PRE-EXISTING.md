...
| `98323f7` `chore: add the receipts testnet demo runner`, `1fc5398` `fix: stop the demo runner when the verdict differs from the scenario` | `scripts/receipts-demo.ts`: the agent-runtime side of the testnet walkthrough; with `--execute` it pays only when the receipt is newly issued and its verdict is the one the scenario expects |

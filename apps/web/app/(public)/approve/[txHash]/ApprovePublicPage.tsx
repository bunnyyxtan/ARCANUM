...
export function ApprovePublicPage({ txHash }: Readonly<{ txHash: string }>) {
  ...
  const escalationManagerAddress = isConfiguredAddress(
  ...
  const quorum = escalation
    ? `${escalation.signaturesCount} / ${escalation.threshold}`
    : "N/A";
          ...
          {!decided && stage !== "checking" && stage !== "wallet" && stage !== "confirming" && (
            <>
              ...
              <p className="mt-4 text-[13px] leading-[1.5] text-[var(--wl-body)]">
                Your decision is signed on-chain and becomes part of the immutable decision record.
          ...
          {(stage === "checking" || stage === "wallet" || stage === "confirming") && (
            <div className="py-12">
              ...
              <p className="mt-4 text-[14px] text-[var(--wl-body)]">
                Recording your decision against {escalationId ? shortAddress(escalationId, { tail: 6 }) : "the request"}.
              </p>
            </div>
          )}
